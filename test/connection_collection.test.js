// Generated by CoffeeScript 1.8.0
(function() {
  var config, inspect, ps, puts, test_helper, _ref;

  _ref = require('util'), puts = _ref.puts, inspect = _ref.inspect;

  ps = require('../lib');

  test_helper = require('./test_helper');

  config = test_helper.config;

  describe('ConnectionCollection', function() {
    beforeEach(function(done) {
      this.config = {
        shards: 4,
        nodes: [
          {
            url: 'tcp://postgres:@localhost/pool_shard_test1',
            shard: {
              min: 1,
              max: 2
            },
            pool_size: 8,
            idle_timeout_millis: 30000
          }, {
            url: 'tcp://postgres:@localhost/pool_shard_test2',
            shard: {
              min: 3,
              max: 4
            },
            pool_size: 8,
            idle_timeout_millis: 30000
          }
        ]
      };
      this.collection = new ps.ConnectionCollection(this.config);
      return done();
    });
    it('should be a ConnectionCollection', function(done) {
      this.collection.should.be.an.instanceOf(ps.ConnectionCollection);
      return done();
    });
    it('should give connections to different db pools & schemas for different shardKeys', function(done) {
      var connectionAlpha, connectionBeta, poolAlpha, poolBeta, schemaAlpha, schemaBeta;
      connectionAlpha = this.collection.connectionFor(1);
      connectionBeta = this.collection.connectionFor(4);
      poolAlpha = connectionAlpha.pool;
      poolBeta = connectionBeta.pool;
      poolAlpha.should.not.equal(poolBeta);
      schemaAlpha = connectionAlpha.schema;
      schemaBeta = connectionBeta.schema;
      schemaAlpha.should.not.equal(schemaBeta);
      return done();
    });
    it('should calculate database & schema using configured number & division of shards and the specified shardKey', function(done) {
      var connectionDelta, poolDelta, schemaDelta, shardKeyDelta;
      shardKeyDelta = 35;
      connectionDelta = this.collection.connectionFor(shardKeyDelta);
      schemaDelta = connectionDelta.schema;
      poolDelta = connectionDelta.pool;
      schemaDelta.should.equal('shard_0003');
      poolDelta.should.equal(this.collection.pools[this.config.nodes[1].url]);
      return done();
    });
    it('should return Connection for specified database & schema', function(done) {
      var connection, databaseName, schemaName;
      databaseName = 'pool_shard_test2';
      schemaName = 'shard_0003';
      connection = this.collection.connectionForSchema(databaseName, schemaName);
      connection.schema.should.equal(schemaName);
      return done();
    });
    it('should return MultiConnection for specified all databases & schemas', function(done) {
      var multiConnection;
      multiConnection = this.collection.connectionForAll();
      multiConnection.poolsAndShards.length.should.equal(this.config.nodes.length);
      return done();
    });
    describe('#createDatabases', function() {
      beforeEach(function() {
        this.adminClients = [];
        this.adminClientOld = ps.ConnectionCollection.adminClient;
        return ps.ConnectionCollection.adminClient = (function(_this) {
          return function(opts) {
            var client;
            client = new test_helper.ClientStub;
            _this.adminClients.push(client);
            return client;
          };
        })(this);
      });
      afterEach(function() {
        return ps.ConnectionCollection.adminClient = this.adminClientOld;
      });
      return it('should create one database for each one specified in the config', function(done) {
        return this.collection.createDatabases((function(_this) {
          return function(err) {
            if (err) {
              throw err;
            }
            _this.adminClients.length.should === 2;
            _this.adminClients[0].queries.length.should === 1;
            _this.adminClients[0].queries[0].should === 'CREATE DATABASE pool_shard_test1';
            _this.adminClients[1].queries.length.should === 1;
            _this.adminClients[1].queries[0].should === 'CREATE DATABASE pool_shard_test2';
            return done();
          };
        })(this));
      });
    });
    return describe('#createDatabases', function() {
      beforeEach(function() {
        var client, databaseName, pool, _ref1, _results, _unused;
        this.stubClients = [];
        this.stubPools = [];
        _ref1 = this.collection.pools;
        _results = [];
        for (databaseName in _ref1) {
          _unused = _ref1[databaseName];
          client = new test_helper.ClientStub;
          pool = new test_helper.PoolStub(client);
          this.stubClients.push(client);
          this.stubPools.push(pool);
          _results.push(this.collection.pools[databaseName] = pool);
        }
        return _results;
      });
      return it('should create one schema for each one specified in the config', function(done) {
        return this.collection.createSchemas((function(_this) {
          return function(err) {
            if (err) {
              throw err;
            }
            _this.stubClients.length.should === 2;
            _this.stubPools.length.should === 2;
            _this.stubClients[0].queries.length.should === 2;
            _this.stubClients[0].queries[0].should === 'CREATE SCHEMA shard_0001';
            _this.stubClients[0].queries[1].should === 'CREATE SCHEMA shard_0002';
            _this.stubClients[1].queries.length.should === 2;
            _this.stubClients[1].queries[0].should === 'CREATE SCHEMA shard_0003';
            _this.stubClients[1].queries[1].should === 'CREATE SCHEMA shard_0004';
            return done();
          };
        })(this));
      });
    });
  });

}).call(this);
